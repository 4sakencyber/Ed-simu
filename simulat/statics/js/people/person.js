...
export function buildPersonPickMeta(t) {
    const floor = Number.isFinite(Number(t.floor)) ? `F${t.floor}` : "Unknown Floor";
    const room = t.room != null ? t.room : "-";
    const region = t.region || "unknown";
    const role = t.is_leader ? "领队" : "普通人员";
    const status = t.finished ? "已完成疏散" : "疏散中";

    return {
        type: "person",
        personId: t.id,
        title: `人员 #${t.id}`,
        detail: `${floor} | 房间 ${room} | 区域 ${region} | ${role} | ${status} | 累积剂量 ${formatDose(t.dose)}`,
        floor: t.floor,
        room,
        region,
        finished: Boolean(t.finished),
        isLeader: Boolean(t.is_leader),
        dose: Number(t.dose) || 0
    };
}
...
export function makePersonMesh(t) {
    ...
    p.userData = {
        ...
        dose: Number(t.dose) || 0,
        clickable: true,
        pickMeta: buildPersonPickMeta(t)
    };
